...
The **Zonnebrand Dashboard** is a live web interface that visualises real-time
...
.. figure:: ../figs/dashboard.png
   ...
   The Zonnebrand Dashboard in dark mode, showing today's EPEX prices with the
...
The dashboard requires **Flask** in addition to the normal ``zonnebrand``
...
To keep the dashboard running alongside ``zonnebrand``, you can start both
processes in parallel, for example with ``tmux`` or ``screen``, or by using
``nohup``:

.. code-block:: console

    # Terminal 1 — run the controller
    python -m zonnebrand --provider zonneplan --mail your_mail@gmail.com
