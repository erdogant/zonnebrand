...
By default the browser runs headless.  Use ``--headless False`` to watch every
Playwright step in a real browser window — handy when troubleshooting
ennexOS login issues.

.. code-block:: console

    python zonnebrand.py --headless False
...
.. code-block:: console
    ...
    python zonnebrand.py --provider anwb --headless False --mail your@email.com
...
.. code-block:: python
    ...
    client = Zonnebrand(
        username=os.getenv("SUNNY_USERNAME"),
        password=os.getenv("SUNNY_PASSWORD"),
        headless=False,       # show the Chromium window
